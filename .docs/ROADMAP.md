...
- [x] **辞書ファイル選択時のパース統合** [M] — .dic ファイル選択時のパースフロー（全 .dic 一括パースに置換済み）
- [x] **カスタムブランチノード** [M] — ノードヘッダー（イベント名）、ダイアログプレビュー、キャラクターラベル（`\0`/`\1` 色分け）、サーフェスサムネイル
- [x] **全 .dic 一括パース** [L] — NAR 読み込み完了時に全 .dic を自動パース、統合 `DicFunction[]` を構築
  - 依存: Web Worker 解析基盤, YAYA 辞書パーサー, Satori 辞書パーサー
- [x] **統合会話グラフ構築** [M] — 統合された `DicFunction[]` を `buildBranchGraph` に渡してグラフ生成。同名関数のダイアログ結合を含む
  - 依存: 全 .dic 一括パース
...
### Phase 4: Conversation Preview — 1/4
目標: 選択したノードの会話内容をプレビューし、パスナビゲーションでインタラクティブに探索する

**対象ディレクトリ:** `src/components/conversation-preview/`
**未インストール依存:** なし

- [x] **会話プレビューパネル** [M] — ノード選択時に右ペインで会話内容を再現表示
  - 依存: ダイアログバリエーション展開
- [ ] **パスナビゲーション** [M] — 選択肢分岐を辿って会話パスを探索
  - 依存: 会話プレビューパネル
- [ ] **サーフェスサムネイル** [S] — `\s[N]` に対応する surface 画像を表示
  - 依存: 会話プレビューパネル
- [ ] **ソースコードジャンプ** [S] — ノードから .dic ファイル・行番号へジャンプ
  - 依存: 会話プレビューパネル
...
- [ ] **SakuraScript シンタックスハイライト定義** [M] — CodeMirror 用カスタム言語定義、キャラ切替 / サーフェス / 選択肢 / イベントの色分け
  - 依存: CodeMirror 6 統合
- [ ] **検索・フィルター** [M] — イベント名フィルタリング、テキスト全文検索
  - 依存: 起点イベント選択 + パスフィルタ
