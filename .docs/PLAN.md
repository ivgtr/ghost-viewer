...
伺か（ukagaka）のゴーストは、SHIORI言語で記述された会話スクリプトによって人格が表現されている。しかし、NARファイル内部の構造は複雑で、会話パターンの全体像を把握するにはファイルを一つ一つ読む必要がある。NARファイルの俯瞰的な分析を可能にするツールが求められている。

NARファイル（Nanika Archive）は実質的にZIPアーカイブであり、内部にゴーストのダイアログスクリプト（.dicファイル）、シェル画像、設定ファイルなどを含む。会話パターンは主にSHIORI言語（YAYA、Satori等）で記述され、イベント駆動型のロジックで会話が生成される。

### 目的

- NARファイルの内部構造をブラウザ上で展開・可視化する
- 会話パターンをブランチ（ツリー）構造で表示し、分岐を確認できるようにする
- 会話パターンの構造的なインサイトを得るための分析ツールを作る
- SHIORI言語の種類（YAYA / Satori）に依存しない統一的な解析アプローチの検討
...
|------|------|
| プロダクト名 | ghost-viewer |
| ターゲット | 伺かゴースト作者、ゴースト分析に関心のある開発者 |
...
| Kawari | 現行方針では非対応（unknown 扱いでスキップ） | 対象外 |
...
1. **SHIORI言語ごとの構造差異**：YAYA / Satori それぞれの会話パターンの特徴
2. **ダイアログ密度の分布**：ゴーストによる会話量の差と、その構造的な傾向
3. **分岐の複雑さ**：選択肢やイベント遷移による会話フローの深さと広がり
4. **エンコーディングの実態**：Shift_JIS / UTF-8 / EUC-JP の使用傾向
